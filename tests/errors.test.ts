...
describe('describeError', () => {
  ...
  it('recognises pdf.js password errors by name', () => {
    ...
    expect(describeError(error, en).kind).toBe('encrypted');
  });
  ...
  it('flags a failure to load the engine as a loading problem', () => {
    expect(describeError(new Error('Failed to fetch the worker script'), en).kind).toBe('assets');
  });

  it('never produces an empty message', () => {
    for (const input of [new Error(''), 'plain string', null, undefined, 42]) {
      const described = describeError(input, en);
  ...
  it("speaks the reader's language", () => {
    const spanish = describeError(new EncryptedPDFError(), es);
    expect(spanish.kind).toBe('encrypted');
    expect(spanish.title).toMatch(/contrase/i);
    expect(spanish.title).not.toBe(describeError(new EncryptedPDFError(), en).title);
  });
});
