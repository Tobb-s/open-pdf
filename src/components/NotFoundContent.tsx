 ...
 *
 * It lives here because two routes need it and neither can be the other. A
 * `notFound()` raised INSIDE a page is caught by `[lang]/not-found.tsx`, which
 * already sits under the layout and its provider. But the locale check that
 * produces most 404s runs in the layout itself, and a boundary cannot catch
 * what its own layout threw — so that case falls through to the root
 * `app/not-found.tsx`, which has to bring its own `<html>` and its own
 * provider. Same page, two entrances.
 */
